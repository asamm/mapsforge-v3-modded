...
public class MapDatabase {
	 ...
	 */
	public boolean readMapData(Tile tile, TileRenderer tr) {
		try {
            // prepare cache
            if (this.databaseIndexCache == null) {
			...
			readNodes =  0;
			readWays = 0;
			return processBlocks(queryParameters, subFileParameter, tr);
		} catch (IOException e) {
	...
	private boolean processBlocks(QueryParameters queryParameters, SubFileParameter subFileParameter, 
	...
	private void processBlock(QueryParameters queryParameters, SubFileParameter subFileParameter, TileRenderer tr) {
...
//			}
	...
	private boolean processPOIs(int numberOfPois, TileRenderer tr) {
		...
		for (int elementCounter = numberOfPois; elementCounter != 0; --elementCounter) {
			...
			tr.renderNode(layer, tags, latitude, longitude);
		}
		return true;
	}
    ...
    private static Comparator<Tag> mTagsComparator = new Comparator<Tag>() {
	...
	private int processWays(QueryParameters queryParameters, int numberOfWays, TileRenderer tr) {
		...
		for (int elementCounter = numberOfWays; elementCounter != 0; --elementCounter) {
			...
			for (byte tagIndex = numberOfTags; tagIndex != 0; --tagIndex) {
				...
				way.tags.add(wayTags[tagId]);
			}

            // sort tags so comparing of ways match
            Collections.sort(way.tags, mTagsComparator);
			...
			if (featureName) {
				String name = extract(readBuffer.readUTF8EncodedString());
				if (name != null) {
					way.tags.add(new Tag(TAG_KEY_NAME_RAW, TAG_KEY_NAME, name));
				}
			}

			// check if the way has a house number
			if (featureHouseNumber) {
				way.tags.add(new Tag(TAG_KEY_HOUSE_NUMBER_RAW, TAG_KEY_HOUSE_NUMBER,
						readBuffer.readUTF8EncodedString()));
			}

			// check if the way has a reference
			if (featureRef) {
				way.tags.add(new Tag(TAG_KEY_REF_RAW, TAG_KEY_REF,
						readBuffer.readUTF8EncodedString()));
			}
