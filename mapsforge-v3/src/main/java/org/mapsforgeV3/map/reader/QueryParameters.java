...
class QueryParameters {
	...
	public boolean equals(QueryParameters qp) {
		if (fromBaseTileX != qp.fromBaseTileX) {
			return false;
		}
		if (fromBaseTileY != qp.fromBaseTileY) {
			return false;
		}
		if (fromBlockX != qp.fromBlockX) {
			return false;
		}
		if (fromBlockY != qp.fromBlockY) {
			return false;
		}
		if (queryTileBitmask != qp.queryTileBitmask) {
			return false;
		}
		if (queryZoomLevel != qp.queryZoomLevel) {
			return false;
		}
		if (toBaseTileX != qp.toBaseTileX) {
			return false;
		}
		if (toBaseTileY != qp.toBaseTileY) {
			return false;
		}
		if (toBlockX != qp.toBlockX) {
			return false;
		}
		if (toBlockY != qp.toBlockY) {
			return false;
		}
		return useTileBitmask == qp.useTileBitmask;
	}
