...
import org.mapsforgeV3.core.model.Rectangle;
...
public class SymbolContainer extends MapElementContainer {
	...
	public void draw(Canvas canvas, Point origin, Matrix matrix) {
		...
		matrix.preTranslate((int) (this.xy.x - origin.x + boundary.left), (int) (this.xy.y - origin.y + boundary.top));
		if (theta != 0 && alignCenter) {
			matrix.preRotate(theta, (float) -boundary.left, (float) -boundary.top);
		} else {
			matrix.preRotate(theta);
		}
		canvas.drawBitmap(this.symbol, matrix, null);
	}
